...
function Test-Tool {
    ...
    if (Test-Command $ToolCommand) {
        if ($ExpectedOutput -ne "") {
            try {
                $Output = & $ToolCommand --version 2>&1
                if ($Output -like "*$ExpectedOutput*") {
                    Write-Info "✓ $ToolName is available and working"
                    $script:ValidationsPassed++
                }
                else {
                    Write-Warn "⚠ $ToolName is available but version check failed: $Output"
                    $script:ValidationsPassed++
                }
            }
            catch {
                Write-Warn "⚠ $ToolName is available but version check failed"
                $script:ValidationsPassed++
            }
        }
        else {
...
function Show-Usage {
    Write-Host @"
Usage: .\validate-security-tools.ps1 [OPTIONS]
...
EXAMPLES:
    .\validate-security-tools.ps1           # Run validation
    .\validate-security-tools.ps1 -Verbose  # Run with verbose output
"@
}
...
function Main {
    ...
    try {
        ...
        Write-Error-Custom "✗ Failed to run security unit tests: $($_.Exception.Message)"
        $script:ValidationsFailed++
    }
